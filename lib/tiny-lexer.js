...
charRef: [
  ...
  { if: CHARREF_LEGACY,   emit: legacyCharRefT,        goto: context       },
...
beforeAtt: [
  ...
  { if: '[\t\n\f ]+',     emit: T.tagSpace,                                },
  { if: '/+(?!>)',        emit: T.tagSpace,                                }, // TODO, test / check with spec
...
afterAttName: [
  ...
  { if: '/+(?!>)',        emit: T.tagSpace,            goto:'beforeAtt'    },
  { if: '[\t\n\f ]+',     emit: T.tagSpace                                 },
...
function compileRow (symbol) {
  return function ({ if:r = '.{0}', emit, goto = symbol }) {
    const g = typeof goto === 'function' ? goto : (symbol, data) => goto
    const e = typeof emit === 'function' ? emit : (symbol, data) => [emit, data]
...
module.exports = { tokenize, tokenise:tokenize, chunks:tokenize, tokenTypes }